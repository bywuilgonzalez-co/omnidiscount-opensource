    ...
    function GlobalSettings({ onBack, onConditionsChange }) {
        ...
        const typesPanel = el('div', { className: 'drw-form-section' },
            ...
            el('div', { className: 'drw-settings-toggles' },
                el(ToggleControl, { label: 'Porcentaje (% de descuento)',         checked: !!(dt.percentage  && dt.percentage.enabled),  onChange: (v) => updateSetting('discount_types.percentage.enabled',  v) }),
                el(ToggleControl, { label: 'Precio fijo ($ de descuento)',        checked: !!(dt.fixed       && dt.fixed.enabled),        onChange: (v) => updateSetting('discount_types.fixed.enabled',       v) }),
